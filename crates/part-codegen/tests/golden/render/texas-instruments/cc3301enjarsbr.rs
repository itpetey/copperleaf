/// Texas Instruments CC3301ENJARSBR SimpleLink Wi-Fi 6 and Bluetooth Low Energy 5.4 Companion IC (WQFN-40)
///
/// Datasheet: https://www.ti.com/lit/gpn/cc3301
///
/// # Pinout
///
...
/// | 2   | RF_BG    | RF I/O      | BLE and WLAN 2.4GHz RF port (RF I/O). WLAN operational frequency 2412-2472 MHz. |
/// | 3   | GND      | Ground      | Ground.               |
...
/// | 11  | UART RTS | I/O         | Device RTS signal, flow control for BLE HCI (output). |
/// | 12  | UART CTS | I/O         | Device CTS signal, flow control for BLE HCI (input). |
/// | 13  | UART RX  | I/O         | UART RX for BLE HCI (input). |
/// | 14  | UART TX  | I/O         | UART TX for BLE HCI (output). |
...
/// | 30  | HOST_IRQ_BLE3 | I/O         | Interrupt request to host for BLE (output). Reserved for future use. |
...
/// | 40  | PA_LDO_IN | Supply      | 3.3V supply for PA (VPA rail). Peak current can reach 340mA during device calibration. |
/// | 41  | EPAD     | Supply      | Exposed thermal pad. Must be soldered to the PCB for thermal and mechanical performance. |
pub struct Cc3301enjarsbr {
    ...
    pub fn new() -> Self {
        ...
        Self {
            pins: vec![
                ...
                Pin::build("GND").number("16").pos(0.200025, 2.5273).rotation(0.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").gnd(),
                ...
                Pin::build("GND").number("20").pos(1.800225, 2.5273).rotation(0.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").gnd(),
                ...
                Pin::build("GND").number("25").pos(2.5273, 0.200025).rotation(90.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").gnd(),
                ...
                Pin::build("GND").number("37").pos(-0.600075, -2.5273).rotation(0.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").gnd(),
                Pin::build("GND").number("38").pos(-1.000125, -2.5273).rotation(0.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").gnd(),
                ...
                Pin::build("PA_LDO_IN").number("40").pos(-1.800225, -2.5273).rotation(0.0).length(0.9144).width(0.2032).height(0.9144).pad_type("smd").pad_shape("rect").layers("F.Cu F.Paste F.Mask").pwr(3.0.volt(), 3.6.volt(), 0.29.amp()).pin(),
...
impl copperleaf::Component for Cc3301enjarsbr {
    ...
    fn meta(&self) -> &copperleaf::ComponentMeta {
        ...
        META.get_or_init(|| copperleaf::ComponentMeta {
            symbol: Some("CC3301ENJARSBR".into()),
            
            footprint: Some("CC3301ENJARSBR".into()),
            
            datasheet: Some("https://www.ti.com/lit/gpn/cc3301".into()),
...
impl Default for Cc3301enjarsbr {
    fn default() -> Self {
        Self::new()
    }
}
