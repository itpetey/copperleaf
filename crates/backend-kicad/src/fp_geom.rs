...
pub fn normalise_anchor(pads: &mut Vec<PadGeom>) {
    ...
    let any_explicit = pads
        .iter()
        .any(|p| p.pin_index.is_some() && p.pos != auto_pad_pos(p.pin_index.unwrap())
            || p.pin_index.is_none());
    if !any_explicit {
    ...
    } else if let Some(anchor) = pads
        .iter()
        .find(|p| p.pin_index == Some(0))
        .map(|p| p.pos)
    {
...
pub fn pad_sexpr(pad: &PadGeom, uuid: Option<&str>, net: Option<(usize, &str)>) -> Sexpr {
    ...
    children.push(Sexpr::list(
        std::iter::once(Sexpr::atom("layers")).chain(
            pad.layers
                .split_whitespace()
                .map(Sexpr::atom),
        ),
...
pub fn fp_line(
    ...
    let mut children = vec![
        ...
        Sexpr::list([
            Sexpr::atom("width"),
            Sexpr::atom(format_float(width, 2)),
        ]),
    ];
...
mod tests {
    ...
    fn outlines_are_on_grid_and_klc_widths() {
        ...
        let text: String = out.iter().map(|s| s.to_string()).collect::<Vec<_>>().join("\n");
        ...
        assert!(!text.contains("0000000000"), "float noise in output: {text}");
    }
}
