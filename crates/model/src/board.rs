...
use crate::erc::{
    erc_floating_inputs, erc_floating_power_inputs, erc_nc_pin_connected, erc_overvoltage,
};
use crate::net::{Constraint, Net, NetHandle, NetKind, NetId};
use crate::pin::{Pin, PinHandle, PinId, PinRef, RawConnection, Role, SigSpec};
use crate::synthesis::synthesize_decoupling;
use crate::units::{Diagnostic, Qty, Severity, UnitExt, Volt};
use crate::util::{deterministic_id, UnionFind};
use crate::Component;
...
fn resolve_net_overrides(
    ...
    for (&rep, members) in &grouping.groups {
        ...
        let (explicit_voltage, explicit_name) =
            merge_overrides(&edge_ids, overrides, errors, rep);
        ...
        let inferred_voltage = infer_voltage_from_pins(members, &grouping.nodes, compiled, &name, errors);
...
fn build_summary(board: &CompiledBoard, synth_caps: Vec<crate::compiled::SynthCap>) -> CompileSummary {
    CompileSummary {
        nets: board
            .nets
            .iter()
