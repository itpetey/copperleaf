...
impl Qty<Hertz> {
    /// Convert a stored frequency (in hertz) to a value in megahertz.
    pub fn as_mhz(&self) -> f64 {
        self.as_base() / 1.0e6
    }
}
...
mod tests {
    ...
    fn hertz_conversions() {
        ...
        assert!((1000.0.khz().as_mhz() - 1.0).abs() < 1e-9);
        assert!((1_000_000.0.hz().as_mhz() - 1.0).abs() < 1e-9);
    }

    #[test]
    fn second_as_mhz_converts_period_to_frequency() {
        let period = (1.0 / 50.0e6).sec();
        assert!((period.as_mhz() - 50.0).abs() < 1e-9);
    }
}
